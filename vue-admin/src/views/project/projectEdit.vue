  ...
  <div class="project-create">
    <el-form ref="form" :model="form" label-position="top">
      ...
      <el-form-item class="btn">
        <el-button type="primary" @click="_edit">修改项目</el-button>
...
<script>
import { getProject, getRecentProject, editProject } from "@/api/api";
export default {
  data() {
    return {
      form: {
  ...
  methods: {
    init() {
      if (this.$route.params.id) {
        getProject({ id: this.$route.params.id }).then((res) => {
          this.form = res.data.project;
        });
      } else {
        getRecentProject().then((res) => {
          this.form = res.data.project[0];
          // 消息提示
          this.$notify.info({
            title: "提醒",
            message:
              "默认修改最近一个项目,您可以通过点击(项目列表),点击(查看or编辑) 修改更多的项目内容哦!",
            duration: 4500,
          });
        });
      }
